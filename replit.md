...
-   **Zoho Inventory Integration (August 2025):** Implemented comprehensive OAuth2 integration with Zoho Inventory API using India domain endpoints. Created secure token storage system with ZohoToken model for access and refresh tokens. Built complete authorization flow with `/zoho-connect` route that redirects to Zoho's authorization page, and `/zoho/callback` route for handling authorization responses. Configured proper scopes for full inventory access including items, sales orders, and webhooks. Integration supports automatic token refresh and connection status checking. All API endpoints use proper India TLD (.in) domain as required. Added comprehensive debugging endpoints (`/zoho/debug`, `/zoho/status`) for troubleshooting OAuth flow and API connectivity. Implemented robust error handling to prevent database constraint violations during failed token exchanges. Fixed callback routing issues and improved security with OAuth state parameters. System ready for real-time inventory synchronization between website and Zoho Inventory once initial OAuth authorization is completed successfully.
...
-   Zoho Inventory API (for real-time inventory management and order synchronization)