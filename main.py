...
def update_cart(variation_id, action):
    ...
    try:
        ...
        # Get updated item details for display
        cursor.execute("""
            SELECT ci.quantity, ci.variation_id,
                   p.name as product_name, v.name as variation_name, 
                   v.price, (ci.quantity * v.price) as total_price
            FROM cart_items ci
            JOIN variations v ON ci.variation_id = v.id
        ...
        # Return complete cart item HTML
        return f'''
